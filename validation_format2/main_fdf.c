...
/*   Updated: 2018/08/05 22:14:00 by tduquesn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			show_debug_info(t_control *ctrl)
{
	...
	mlx_string_put(ctrl->mlx_ptr, ctrl->win_ptr,
		HALF_DRENWIN_WIDTH + 20, HALF_DRENWIN_HEIGHT + 80, WHITE, str);
	ft_strdel(&str);
	...
	mlx_string_put(ctrl->mlx_ptr, ctrl->win_ptr, HALF_DRENWIN_WIDTH + 20,
		HALF_DRENWIN_HEIGHT + 100, GREEN, str);
}
...
int				main(int argc, char **argv)
{
	...
	if (argc != 2)
		exit_error("usage: \"./fdf [filepath/to/fdf/map.fdf]\"", 0);
	init_mlx(&ctrl);
	ctrl.debug = 0;
